...
export default function NewDiagnosisPage() {
  ...
  const handleFile = useCallback((f: File) => {
    ...
    if (ext !== "xlsx" && ext !== "xls") {
      toast({ title: t("invalid_file"), description: t("only_xlsx"), variant: "destructive" });
      return;
    }
    setFile(f);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res1.status !== 201) {
        toast({ title: t("error_creating"), variant: "destructive" });
        return;
      }
      const session = await res1.json();
      const formData = new FormData();
      formData.append("file", file);
      const res2 = await apiFetch(`/sessions/${session.id}/upload-excel`, { method: "POST", body: formData });
      if (res2.ok) {
        toast({ title: t("session_created") });
        navigate(`/chat/${session.id}`);
      } else {
        toast({ title: t("error_uploading"), variant: "destructive" });
      }
    } catch {} finally {
      setLoading(false);
    }
  ...
  const steps = [
    { labelKey: "step_engine", done: !!engineType },
    { labelKey: "step_fault", done: !!problemDesc.trim() },
    { labelKey: "step_history", done: !!file },
  ];
    ...
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-3xl animate-fade-in">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-foreground mb-1">{t("new_diagnosis")}</h1>
            <p className="text-sm text-muted-foreground">{t("new_diag_sub")}</p>
          </div>
          ...
          {steps.map((s, i) => (
            <div key={s.labelKey} className="flex items-center gap-2">
              <div className={`flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-mono-tech tracking-[0.12em] border transition-colors ${
                s.done ? 'border-success/30 text-success bg-success/5' : 'border-border text-muted-foreground/50'
              }`}>
        ...
        <div className="space-y-6">
          ...
          <div className="relative">
            <label className="label-tech">{t("engine_type_label")}</label>
            <input
              className="input-field"
              value={engineSearch || engineType}
              onChange={(e) => { setEngineSearch(e.target.value); setEngineType(e.target.value); setShowDropdown(true); }}
              onFocus={() => setShowDropdown(true)}
              onBlur={() => setTimeout(() => setShowDropdown(false), 200)}
              placeholder={t("engine_search_ph")}
            />
              ...
              >
                {filtered.map((e) => (
                  <button
                    ...
                    onMouseDown={() => { setEngineType(e); setEngineSearch(e); setShowDropdown(false); }}
                  >
          ...
          {/* Problem description */}
          <div>
            <label className="label-tech">{t("problem_desc")}</label>
            <textarea
              className="input-field min-h-[140px] resize-y"
              value={problemDesc}
              onChange={(e) => setProblemDesc(e.target.value)}
              placeholder={t("problem_ph")}
            />
            <p className="font-mono-tech text-[10px] text-muted-foreground/40 mt-1.5 tracking-wider">
              {problemDesc.length} {t("characters")}
            </p>
          </div>

          {/* File upload */}
          <div>
            <label className="label-tech">{t("maintenance_history")}</label>
            {!file ? (
              <div
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mb-1">{t("drag_drop")}</p>
                <p className="text-xs text-muted-foreground/50">{t("xlsx_format")}</p>
              ...
              <motion.div className="surface-panel overflow-hidden" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          >
            {loading ? (
              ...
              <>{t("start_diagnosis")} <ChevronRight className="w-4 h-4" /></>
            )}
          </motion.button>
        </div>
      </div>
