...
export default function DashboardPage() {
  ...
  const stats = [
    { labelKey: "stat_total", value: total, icon: Activity, color: "text-foreground" },
    { labelKey: "stat_resolved", value: resolved, icon: CheckCircle, color: "text-success" },
    { labelKey: "stat_active", value: active, icon: Clock, color: "text-warning" },
  ];
      ...
      <div className="p-6 md:p-8 max-w-6xl animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            <Plus className="w-4 h-4" /> {t("new_diagnosis")}
          </motion.button>
        </div>
          ...
          {stats.map((s, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="font-mono-tech text-[9px] text-muted-foreground/40 tracking-[0.15em]">{t(s.labelKey)}</span>
              </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-mono-tech text-[10px] text-muted-foreground/60 tracking-[0.15em]">{t("recent_sessions")}</h2>
            <span className="font-mono-tech text-[10px] text-muted-foreground/40">{total} {t("stat_total")}</span>
          </div>
          ...
          {loading ? (
            ...
            <motion.div className="surface-panel p-12 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <Plane className="w-10 h-10 text-muted-foreground/20 mx-auto mb-4" />
              <p className="text-muted-foreground mb-1">{t("no_sessions")}</p>
              <p className="text-xs text-muted-foreground/60 mb-6">{t("start_first")}</p>
              <button onClick={() => navigate("/new-diagnosis")} className="btn-aerospace text-sm inline-flex items-center gap-2">
                <Plus className="w-4 h-4" /> {t("start_diagnosis")}
              </button>
            </motion.div>
          ) : (
