  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        toast({ title: "Document uploaded", description: `${data.filename} — ${data.chunks_created} chunks created` });
        setFile(null);
        setEngineTag("");
        fetchDocs();
  ...
  return (
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-4xl animate-fade-in">
        <h1 className="text-2xl font-bold text-foreground mb-1">Upload AMM Document</h1>
        <p className="text-sm text-muted-foreground mb-8">Upload Aircraft Maintenance Manual PDFs for AI reference during diagnostics.</p>

        <div className="surface-panel p-6 space-y-5 mb-10">
          <div>
            <label className="label-tech">ENGINE TYPE TAG</label>
            <input className="input-field" value={engineTag} onChange={(e) => setEngineTag(e.target.value)} placeholder="e.g. CFM56-7B" />
          </div>
          <div>
            <label className="label-tech">PDF FILE</label>
            {!file ? (
              <div
              ...
              <motion.div
                className="flex items-center gap-3 px-4 py-3 surface-elevated"
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-mono-tech text-[10px] text-muted-foreground/60 tracking-[0.15em]">EXISTING DOCUMENTS</h2>
          ...
          <div className="space-y-1.5">
            {docs.map((d, i) => (
              <motion.div
                key={d.id}
                className="surface-panel px-4 py-3 flex items-center gap-4"
